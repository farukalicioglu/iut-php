<header id="header" class="header-bordered header-transparent transparent-light">
		<div class="header-inner clearfix">

            <!-- LOGO -->
            <div id="logo" class="left-float">
                <a href="../EN/">
                	<img id="scroll-logo" src="../files/iu-logo-light.png" srcset="../files/iu-logo-light.png 1x, ../files/iu-logo-light.png 2x" alt="Logo Scroll">
                	<img id="dark-logo" src="../files/iu-logo-light.png" srcset="../files/iu-logo-light.png 1x, ../files/iu-logo-light.png 2x" alt="Logo Dark">
                	<img id="light-logo" src="../files/iu-logo-light.png" srcset="../files/iu-logo-light.png 1x, ../files/iu-logo-light.png 2x" alt="Logo Light">
                </a>
            </div>

            <!-- MAIN NAVIGATION -->
            <div id="menu" class="right-float clearfix">
            	<a href="#" class="responsive-nav-toggle"><span class="hamburger"></span></a>
                <div class="menu-inner">
                    <nav id="main-nav">
                      <ul>
                        <li><a href="../EN/?p=Hakkimizda">ABOUT US</a></li>
						  <li class="menu-item-has-children"><a href="javascript:void(0);">CATEGORIES</a>
								<ul class="submenu">
									<li><a href="../EN/?p=iu-100K">IU-100K</a><div></div></li>
									<li><a href="../EN/?p=iu-60K">IU-60K</a><div></div></li>
									<li><a href="../EN/?p=iu-30K">IU-30K</a><div></div></li>
									<li><a href="../EN/?p=iu-15K">IU-15K</a><div></div></li>
								</ul>
							</li>
                        <li><a href="../EN/?p=Results">RESULTS</a></li>
                        <li><a href="https://uzunetap.com/Registration/?lang=en" target="_blank">REGISTRATION</a></li>
                        <li><a href="../EN/?p=Contact">CONTACT</a></li>
                        <li style="margin-left:20px;"> | <a href="../EN/">TR</a></li>
                      </ul>
                    </nav>
                </div><!-- END .menu-inner -->
            </div><!-- END #menu -->

		</div> <!-- END .header-inner -->
	</header>